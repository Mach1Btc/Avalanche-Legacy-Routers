...
import {
  ...
  BLACKHOLE_ROUTER_ABI,
  WAVAX_ADDRESS,
  BLACKHOLE_FACTORY_ADDRESS,
  BLACKHOLE_FACTORY_ABI,
} from "@/lib/constants";
...
export async function createSwapTransaction(
  ...
  try {
    ...
    const txHash = await writeContract(web3Config, {
      address: BLACKHOLE_ROUTER_ADDRESS,
      abi: BLACKHOLE_ROUTER_ABI,
      functionName: method,
      args,
...
export const getPairAddressFor = async (
  ...
  try {
    ...
    const pairContractAddress = await readContract(web3Config, {
      address: BLACKHOLE_FACTORY_ADDRESS as `0x${string}`,
      abi: BLACKHOLE_FACTORY_ABI,
