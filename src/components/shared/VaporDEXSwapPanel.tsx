...
import { defaultSlippage, explorer_url, VAPOR_ROUTER_ADDRESS, safeModeEnabledMaxSlippage, WAVAX_ADDRESS } from '@/lib/constants';
    ...
    useEffect(() => {
        const getFromTokenAllowance = async () => {
            ...
            if (WrapUtils.needsAllowance(fromToken.address) && needsAllowanceCheck) {
                const allowance = await getERC20Allowance(account.address, VAPOR_ROUTER_ADDRESS, fromToken.address);
                if (allowance !== null) {
                        ...
                        <div className='flex-1 flex flex-col px-3'>
                            ...
                            <div className='w-full p-2'>
                                <Button
                                    ...
                                    className="vapor-dex-swap-button"
                                    onClick={() => {
                                        if (isConnected) {
                                ...
                                <SwapInfo routerAddress={VAPOR_ROUTER_ADDRESS} pairAddress={currentPairExists ? currentPairAddress : undefined} mainSiteLink='https://app.vapordex.io/swap'/>
                            </div>
